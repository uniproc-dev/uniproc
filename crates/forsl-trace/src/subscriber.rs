...
pub fn init_subscriber<W>(writer: W, dump_capacity: usize, targets: Targets) -> anyhow::Result<()>
where
    W: for<'writer> MakeWriter<'writer> + Send + Sync + 'static,
{
    init_internal(writer, dump_capacity, targets, None)
}
...
where
    W: for<'writer> MakeWriter<'writer> + Send + Sync + 'static,
{
    init_internal(writer, dump_capacity, targets, Some(test_storage))
}
